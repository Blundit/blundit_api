module Api::V1
  class ClaimsController < ApiController
    ...
    def index
      ...
      @claims = Claim.do_search(params[:query], params[:sort], params[:status]).includes(:experts).includes(claim_categories: :category).includes(:categories).page(current_page).per(per_page)
    ...
    def show
      ...
      if current_user.nil?
        ...
        @user_vote = @claim.all_votes.where({user_id: current_user.id}).last
    ...
    def override
      if !params.has_key?(:claim_id) or !params.has_key?(:vote_value) or !params.has_key?(:reason)
        render json: { result: "claim_id and value are both required" }, status: 422
        return
      end
      ...
      @v.vote_value = params[:vote_value]
